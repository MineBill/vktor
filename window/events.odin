...
setup_events :: proc(window: ^Window, parent_context := context) {
    window.event_context.odin_context = parent_context
    glfw.SetWindowUserPointer(window.handle, &window.event_context)

    glfw.SetCharCallback(window.handle, glfw_char_callback)
    glfw.SetWindowSizeCallback(window.handle, glfw_window_size_callback)
    glfw.SetKeyCallback(window.handle, glfw_key_callback)
    glfw.SetMouseButtonCallback(window.handle, glfw_mouse_button_callback)
    glfw.SetCursorPosCallback(window.handle, glfw_cursor_pos_callback)
}
...
glfw_cursor_pos_callback :: proc "c" (win: glfw.WindowHandle, x, y: f64) {
	...
	append(&state.events, MousePositionEvent{x, y})
}
